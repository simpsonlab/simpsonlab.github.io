...
The core of ONT sequencing data consists of electrical current level measurements (in picoamps) taken while biological molecules (DNA, and more recently, RNA) are threaded through a sequencing pore. The process of basecalling translates these currents into more usual base pairs. However, current levels are arguably richer than just base pairs. For example, in [recent work](http://www.nature.com/nmeth/journal/vaop/ncurrent/full/nmeth.4184.html), we demonstrate how current levels (more specifically, event-level data, as detailed below) can be used to detect methylation. For this reason, when trying to reduce the size of fast5 data, it might be desirable to archive current levels in addition to, or independent of, base pairs.

### A New Tool

Motivated by these concerns, we introduce a new tool `f5pack` for the purpose packing fast5 files. This tool is part of the [fast5 library](https://github.com/mateidavid/fast5), more specifically, it is distributed with the Python wrapper of this library. Using this tool, we were able to reduce the storage required by 10,000 ONT NA12878 reads by a factor of ~10. Here is the detailed log of this packing run:

```
bp_single_count         92857415                  # bases sequenced
rs_count                2483038052                # count of raw samples
rs_bits                 16331772672  6.58  175.88
rs_total_bits           16331772672  6.58  175.88
ede_count               451912927                 # count of eventdetection events
ede_skip_bits           452604512    1.00  4.87
ede_len_bits            3243960992   7.18  34.93
ede_total_bits          3696565504   8.18  39.81
bp_count                95063899                  # count of bases in fastq entries
bp_bits                 218731072    2.30  2.36
qv_bits                 547456248    5.76  5.90
bp_total_bits           766187320    8.06  8.25
bce_count               369403783                 # count of basecall events
bce_rel_skip_bits       2111901000   5.72  22.74
bce_move_bits           464575320    1.26  5.00
bce_p_model_state_bits  738838088    2.00  7.96
bce_total_bits          3315314408   8.97  35.70
rs_total_duration       620760.76                 # total duration spanned by raw samples, in seconds
rs_called_duration      597422.90                 # duration of raw samples that were basecalled
rs_frac_called          0.96
bp_per_sec              155.43                    # average sequencing speed
input_bytes             31950088342
output_bytes            3361740258                # size of dataset encodings
output_overhead_bytes   348010270                 # size of output minus size of dataset encodings
```
...
- **Event-level data** aggregates raw samples data into *events*, each of which ideally corresponds to a specific DNA context found in the pore. To understand this, first observe that DNA is threaded through the sequencing pore using a biological process at a stochastic rate. Since raw samples are measured at a fixed rate, the number of raw samples corresponding to a specific DNA context is a stochastic quantity. The process of *event detection* takes as input the sequence of raw samples, and it produces a sequence of events. Ideally, each event corresponds to exactly on DNA context, but the process is noisy, so some DNA contexts can correspond to either 0 events (i.e., they are missed) or 2 or more events (i.e., they are over-separated). Fast5 files might contain both *pre-basecall* and *post-basecall* event data.
  ...
  - `start`, `length`: indexes in the raw samples sequence,
  ...
  - `move`: the number of bases that changed from the DNA context in the previous event,
  ...
  - `start` is encoded as `skip`, a difference from previous event end. `skip` is usually 0, but not always, with non-0 values corresponding to instances where the event detection process decided to skip 1 or more raw samples for various reasons (e.g., perhaps they were too error-prone to decode). On this dataset we achieve 1.00 bits per event (see `ede_skip_bits` above).
  - `length` is encoded using a Huffman code based on a predetermined distribution of values, computed from several training files. On this dataset we achieve 7.18 bits per event (see `ede_len_bits` above).
  ...
  - When both pre- and post-basecall events are present, for each post-basecall event we also encode `rel_skip`, the relative skip in post-basecall events with respect to the pre-basecall event indices. This value is 0 if no pre-basecall events are skipped, but it can be non-zero as well. On this dataset we achieve 5.72 bits per event (see `bce_rel_skip_bits` above).
  - `move` is encoded using a Huffman code based on a predetermined distribution of values, computed from several training files. On this dataset we achieve 1.26 bits per event (see `bce_move_bits` above).
  - `state` is not coded at all. Instead, `f5pack` requires the existence of base-level data (packed or not) to pack event-level data, and it recomputes `state` using `move` as part of the unpacking process.
  - `p_model_state` is a probability, and `f5pack` exposes an option as to how many bits of precision to encode from it, defaulting to 2 bits (see `bce_p_model_state_bits` above).
...
- **Base-level data** is produced by the basecalling process, and it consists of [fastq](https://en.wikipedia.org/wiki/FASTQ_format) entries. For each base pair, the fastq format stores the base itself, and a quality value. We encode these as follows:
  ...
  - The bases are encoded using a Huffman code in which we allow for non-ACGT bases. On this dataset, we achieve 2.30 bits per base (see `bp_bits` above).
  - For the base quality value, `f5pack` exposes an option as to how many bits of precision to encode from it, defaulting to 5 bits (see `qv_bits` above).
...
- Input: `f5pack` accepts as inputs any combination of individual fast5 files, directories, or files of fast5 file names. If no input is given, a file of fast5 file names is read fronm stdin. Input directories are traversed recursively with `--recursive`.
...
- Presets. `f5pack` recognizes the following presets:
  ...
  To archive raw samples data, then restore it in order to rerun Metrichor, try:

  ```
  f5pack --archive --output pack_rs input      # pack raw samples, drop everything else
  f5pack --unpack --output unpack_rs pack_rs   # unpack raw samples
  # now run Metrichor on files in unpack_rs/
  ```

- Tweak what is packed:

  - `--qv-bits`: Number of (most significant) fastq base quality value bits to store. Default: 5.
  - `--p-model-state-bits`: Number of bits to keep from `p_model_state`. Default: 2.

### Compatibility

There are several different existing ONT basecallers: Metrichor, MinKNOW (local), Albacore, Nanonet, and possibly others. Unfortunately, the format for event-level data is not standardized, and as such they each encode it in slightly different ways. We used the first 3, and Metrichor is the only one that produces both pre- and post-basecalling event data, while MinKNOW and Albacore produce only post-basecalling event data. `f5pack` is designed to work with both pre-and-post and post-only event-level data.

However, currently MinKNOW and Albacore both contain a show-stopper issue from the point of view of `f5pack`, in that they do not store enough bits to allow the unambiguous reconstruction of a raw samples index from the event `start` field. We reported this issue to ONT, but pending a design change, **Metrichor is the only basecaller for which we support packing event-level data**.
